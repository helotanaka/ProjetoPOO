...
public class MediatorOperacao {
    ...
    public String realizarOperacao(boolean ehAcao, int entidadeCredito, int entidadeDebito, int idAcaoOuTitulo, double valor) {
        ...
        if (credito == null) {
            return "Entidade de crédito inexistente";
        }

        EntidadeOperadora debito = mediatorEntidadeOperadora.buscar(entidadeDebito);
        if (debito == null) {
            return "Entidade de débito inexistente";
        }

        if (ehAcao && (!credito.getAutorizacaoAcao() || !debito.getAutorizacaoAcao())) {
            return "Entidade não autorizada para ação";
        }
        ...
        double valorOperacao = valor; // Use direct value for debt titles, as no additional calculation is defined.
        ...
        if (mediatorEntidadeOperadora.alterar(credito) != null || mediatorEntidadeOperadora.alterar(debito) != null) {
            return "Falha ao atualizar entidades";
        }

        Transacao transacao = new Transacao(credito, debito, ehAcao ? (Acao) acaoOuTitulo : null, !ehAcao ? (TituloDivida) acaoOuTitulo : null, valorOperacao, LocalDateTime.now());
